...
import type { Besitzer, Medien, CreateBesitzer, CreateMedien } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- MEDIEN ---
  static async getMedien(): Promise<Medien[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MEDIEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Medien[];
    return enrichLookupFields(records, 'medien');
  }
  static async getMedienEntry(id: string): Promise<Medien | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MEDIEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Medien;
    return enrichLookupFields([record], 'medien')[0];
  }
  static async createMedienEntry(fields: CreateMedien) {
    return callApi('POST', `/apps/${APP_IDS.MEDIEN}/records`, { fields: cleanFieldsForApi(fields as any, 'medien') });
  }
  static async updateMedienEntry(id: string, fields: Partial<CreateMedien>) {
    return callApi('PATCH', `/apps/${APP_IDS.MEDIEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'medien') });
  }
  static async deleteMedienEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MEDIEN}/records/${id}`);
  }

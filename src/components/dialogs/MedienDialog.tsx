...
interface MedienDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Medien['fields']) => Promise<void>;
  defaultValues?: Medien['fields'];
  besitzerList: Besitzer[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function MedienDialog({ open, onClose, onSubmit, defaultValues, besitzerList, enablePhotoScan = true, enablePhotoLocation = true }: MedienDialogProps) {
  const [fields, setFields] = useState<Partial<Medien['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'medien');
      await onSubmit(clean as Medien['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "menu_name": string | null, // Medientitel\n  "beschreibung": string | null, // Beschreibung\n  "kategorie": LookupValue | null, // Kategorie (select one key: "buch" | "zeitschrift" | "film" | "musik" | "sonstiges") mapping: buch=Buch, zeitschrift=Zeitschrift, film=Film, musik=Musik, sonstiges=Sonstiges\n  "preis": number | null, // Preis (€)\n  "verfuegbar": boolean | null, // Verfügbar\n  "besitzer": string | null, // Display name from Besitzer (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Medien['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Medien bearbeiten' : 'Medien hinzufügen';

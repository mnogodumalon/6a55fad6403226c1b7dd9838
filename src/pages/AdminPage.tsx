...
import { MedienViewDialog } from '@/components/dialogs/MedienViewDialog';
...
const MEDIEN_FIELDS = [
  { key: 'menu_name', label: 'Medientitel', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'buch', label: 'Buch' }, { key: 'zeitschrift', label: 'Zeitschrift' }, { key: 'film', label: 'Film' }, { key: 'musik', label: 'Musik' }, { key: 'sonstiges', label: 'Sonstiges' }] },
...
const ENTITY_TABS = [
  ...
  { key: 'medien', label: 'Medien', pascal: 'Medien' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'medien': return (data as any).medien as Medien[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'medien' && fieldKey === 'besitzer') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'medien': return MEDIEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'medien': return {
        create: (fields: any) => LivingAppsService.createMedienEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMedienEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteMedienEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'medien' || dialogState?.entity === 'medien') && (
        <MedienDialog
          open={createEntity === 'medien' || dialogState?.entity === 'medien'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'medien' ? handleUpdate : (fields: any) => handleCreate('medien', fields)}
          defaultValues={dialogState?.entity === 'medien' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'medien' && (
        <MedienViewDialog
          open={viewState?.entity === 'medien'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'medien', record: r }); }}
          besitzerList={(data as any).besitzer ?? []}
        />
      )}
